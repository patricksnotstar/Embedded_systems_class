...
{
    // The bug is here, the loop starts from 9 and should stop at 0
    // but because i is an unsigned int so i-- when i = 0 will result in an underflow (0-1)
    // causing i to become the max unsigned int value
    int i;
    for (i = size - 1; i >= 0; i--)
    {
